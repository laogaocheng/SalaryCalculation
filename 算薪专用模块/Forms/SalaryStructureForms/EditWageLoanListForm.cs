...
{
    public partial class EditWageLoanListForm : XtraForm
    {
        bool showDifferent = false;
        bool isCheck = false;
        protected List<WageLoanEntry> currEntryRows = new List<WageLoanEntry>();
        protected WageLoanEntry currWageLoanEntry = null;//当前记录

        public EditWageLoanListForm(bool isCheck)
        {
        ...
        {
            this.Text = "员工工资借款录入 - " + (isCheck  ? "验证录入" : "初次录入");
            showDifferent = false;
            LoadData();
        }
