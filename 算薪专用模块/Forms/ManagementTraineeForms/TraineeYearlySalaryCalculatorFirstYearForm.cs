...
{
    public partial class TraineeYearlySalaryCalculatorFirstYearForm : XtraForm
    {        
        ...
        public TraineeYearlySalaryCalculatorFirstYearForm(string division, string grade)
            : this()
        {
            this.division = division;
            this.year = Convert.ToInt32(division);
            this.grade = grade;         
        }

        public TraineeYearlySalaryCalculatorFirstYearForm()
        {
        ...
        {
            lbl标题.Text = division + " 届定职人员（" + grade + "）" + year + "年度月薪计算";
            gridBand届别.Caption = division + " 年";
            LoadData();
        }
        ...
        private void advBandedGridView1_CustomColumnDisplayText(object sender, CustomColumnDisplayTextEventArgs e)
        {
            if (e.Column.FieldName.EndsWith("年薪") && e.Value != null)
            {
                ...
                if (v > 0) e.DisplayText = v.ToString("#0.##") + " 万元";
            }
            if (e.Column.FieldName.EndsWith("月薪") && e.Value != null)
            {
                int v = (int)e.Value;
                if (v > 0) e.DisplayText = v.ToString() + " 元";
            }
        }
        ...
        {
            TraineeYearlySalaryCalculatorForm form = new TraineeYearlySalaryCalculatorForm(division, grade, year + 1);
