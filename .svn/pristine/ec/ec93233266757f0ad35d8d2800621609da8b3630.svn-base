...
{
    public partial class EditPersonReimbursementForm : XtraForm
    {
        protected bool isCheckInput = false; //是否验证录入
        protected List<PersonReimbursementInput> currInputRows = new List<PersonReimbursementInput>();
        protected PersonReimbursementInput currPersonReimbursement = null;//当前记录

        public EditPersonReimbursementForm(bool isCheck) 
            : this()
        {
            isCheckInput = isCheck;
        }

        public EditPersonReimbursementForm()
        {
        ...
        private void EditPersonReimbursementForm_Load(object sender, EventArgs e)
        {
            this.Text = "员工报销录入 - " + (this.是验证录入 ? "验证录入" : "初次录入");
        ...
        {
            //清除原来的数据
            currInputRows = PersonReimbursementInput.GetEditingRows(Convert.ToInt32(year.EditValue), Convert.ToInt32(month.EditValue), this.是验证录入);
        ...
        private void BecomeEffective(PersonReimbursementInput input)
        {
            PersonReimbursement m = new PersonReimbursement();
            PersonReimbursement found = PersonReimbursement.GetPersonReimbursement(input.员工编号, input.项目, input.年, input.月);
            if (found != null)
            {
                m = found;
            }
            ...
            PersonReimbursementInput anotherInput = input.另一人录入的记录 as PersonReimbursementInput;
        ...
        private void btn保存_Click(object sender, EventArgs e)
        {
            ...
            if (colView != null)
            {
                PersonReimbursementInput currentItem = (PersonReimbursementInput)colView.GetFocusedRow();
                if (currentItem != null)
                {
                ...
                //遍历
                foreach(PersonReimbursementInput row in currInputRows)
                {
            ...
            MyHelper.WriteLog(LogType.信息, "修改员工报销记录", null);
        }
        #endregion
        ...
        {
            PersonReimbursementInput item = new PersonReimbursementInput();
            ...
            MyHelper.WriteLog(LogType.信息, "新增员工报销费用录入记录", item.ToString<PersonReimbursementInput>());
        }
        #endregion
        ...
        private void btn删除_Click(object sender, EventArgs e)
        {
            ...
            if (colView != null)
            {
                if (MessageBox.Show("确实删除当前记录吗？", "删除提示", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button2, 0, false) == DialogResult.Yes)
                {
                    ...
                    MyHelper.WriteLog(LogType.信息, "删除员工报销标准录入记录", currentItem.ToString<PersonReimbursementInput>());
                    gridControl1.RefreshDataSource();
                    currentItem.Delete();
        ...
        {
            PersonReimbursementInput row = gridView1.GetRow(e.RowHandle) as PersonReimbursementInput;
        ...
        {
            PersonReimbursementInput row = gridView1.GetRow(e.PrevFocusedRowHandle) as PersonReimbursementInput;
            if (row != null)
            {                
                row.GetModifiyFields();
            }
