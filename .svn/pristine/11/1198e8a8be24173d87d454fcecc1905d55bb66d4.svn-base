...
{
    public partial class EditContractAllowanceListForm : XtraForm
    {
        bool showDifferent = false;
        bool isCheck = false;
        protected List<ContractAllowanceEntry> currEntryRows = new List<ContractAllowanceEntry>();
        protected ContractAllowanceEntry currContractAllowanceEntry = null;//当前记录

        public EditContractAllowanceListForm(bool isCheck)
        {
        ...
        {
            this.Text = "录入员工契约津贴标准 - " + (isCheck  ? "验证录入" : "初次录入");
            showDifferent = false;
            LoadData();
        }
        ...
        protected void LoadData()
        {
            showDifferent = false;
            //清除原来的数据
            currEntryRows = ContractAllowanceEntry.GetEditingRows(isCheck);
        ...
        private void Submit()
        {
            ...
            List<ContractAllowanceEntry> ssList = ContractAllowanceEntry.GetEditingRows(false);
            List<ContractAllowanceEntry> ssList_opposite = ContractAllowanceEntry.GetEditingRows(true);
            //检查是否录入完成
            foreach (ContractAllowanceEntry wle in ssList)
            {
        ...
        {
            ContractAllowanceEntry item = new ContractAllowanceEntry();
            ...
            MyHelper.WriteLog(LogType.信息, "新增员工工资借款录入记录", item.ToString<ContractAllowanceEntry>());
        }
        #endregion
        ...
        private void btn删除_Click(object sender, EventArgs e)
        {
            ...
            if (colView != null)
            {
                ...
                {
                    ContractAllowanceEntry currentItem = (ContractAllowanceEntry)colView.GetFocusedRow();
                    currEntryRows.Remove(currentItem);
                    MyHelper.WriteLog(LogType.信息, "删除员工工资借款录入记录", currentItem.ToString<ContractAllowanceEntry>());
                    gridControl1.RefreshDataSource();

                    ContractAllowanceEntry.ClearContractAllowanceEntry(currentItem.员工编号);
        ...
        private void bandedGridView1_CustomDrawCell(object sender, RowCellCustomDrawEventArgs e)
        {
            ...
            ContractAllowanceEntry row = bandedGridView1.GetRow(e.RowHandle) as ContractAllowanceEntry;
            if (row != null)
            {
        ...
        {
            ContractAllowanceEntry row = bandedGridView1.GetRow(e.PrevFocusedRowHandle) as ContractAllowanceEntry;
            if (row != null)
            {                
        ...
        {
            ContractAllowanceEntry item = bandedGridView1.GetRow(bandedGridView1.FocusedRowHandle) as ContractAllowanceEntry;
        ...
        private void Edit(ContractAllowanceEntry item)
        {
            EditContractAllowanceForm form = new EditContractAllowanceForm(item);
            //如果录入成功
