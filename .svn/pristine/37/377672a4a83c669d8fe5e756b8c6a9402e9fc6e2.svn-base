...
{
    [Persistent("员工职级_录入")]
    public partial class EmpSalaryStepInput : XPLiteObject
    {
        Guid f标识;
        [Key(true)]
        public Guid 标识
        ...
        public int 薪级标识
        {
            ...
            set
            {
                SetPropertyValue<int>("薪级标识", ref f薪级标识, value);
                //stepName = SalaryNode.GetName(value);
            }
        }
        DateTime f执行日期;
        [WatchMember]
        ...
        public EmpSalaryStepInput(Session session) : base(session) { }
        public EmpSalaryStepInput() : base(MyHelper.XpoSession) { }
