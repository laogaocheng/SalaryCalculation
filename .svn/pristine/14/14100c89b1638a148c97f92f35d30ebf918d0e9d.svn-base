...
{
    public partial class WageLoanCalculatorForm : XtraForm
    {
        protected List<MonthlyWageLoanItem> all_rows = new List<MonthlyWageLoanItem>();

        public WageLoanCalculatorForm()
        {
        ...
        {
            all_rows = MonthlyWageLoanItem.GetMonthlyWageLoanItems(Convert.ToInt32(year.Value), Convert.ToInt32(month.EditValue));
            all_rows = all_rows.OrderByDescending(a => a.月借款标准).ThenBy(a => a.部门).ThenBy(a => a.职务).ToList();
        ...
        {
            WageLoan.AutoGenerateMonthlyWageLoanItems(Convert.ToInt32(year.Value), Convert.ToInt32(month.EditValue));
            LoadData();
        }
    }
