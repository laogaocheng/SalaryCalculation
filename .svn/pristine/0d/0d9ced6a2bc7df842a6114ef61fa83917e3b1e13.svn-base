...
{
    public partial class EditMonthlyWageLoanItemListForm : XtraForm
    {
        bool showDifferent = false;
        bool isCheck = false;
        int year;
        int month;
        protected List<MonthlyWageLoanItemEntry> currEntryRows = new List<MonthlyWageLoanItemEntry>();
        protected MonthlyWageLoanItemEntry currMonthlyWageLoanItemEntry = null;//当前记录

        public EditMonthlyWageLoanItemListForm(bool isCheck)
        {
        ...
        {
            this.Text = "员工每月工资借款录入 - " + (isCheck  ? "验证录入" : "初次录入");
            showDifferent = false;
            LoadData();
        }
        ...
        protected void LoadData()
        {
            showDifferent = false;
            //清除原来的数据
            currEntryRows = MonthlyWageLoanItemEntry.GetEditingRows(year, month, isCheck);
        ...
        private void Submit()
        {
            ...
            List<MonthlyWageLoanItemEntry> ssList = MonthlyWageLoanItemEntry.GetEditingRows(year, month, false);
            List<MonthlyWageLoanItemEntry> ssList_opposite = MonthlyWageLoanItemEntry.GetEditingRows(year, month, true);
            //检查是否录入完成
            foreach (MonthlyWageLoanItemEntry mrs in ssList)
            {
                if(mrs.实际借款金额 == 0)
                {
            ...
            {
                foreach (MonthlyWageLoanItemEntry wle in ssList)
                {
        ...
        private void btn添加_Click(object sender, EventArgs e)
        {
            ...
            //获取正在执行的报账工资标准
            List<WageLoan> wlList = WageLoan.GetEffectiveWageLoans();
            foreach (WageLoan wl in wlList)
            {
                EmployeeInfo emp = wl.员工信息;
        ...
        private void btn删除_Click(object sender, EventArgs e)
        {
            ...
            if (colView != null)
            {
                ...
                {
                    MonthlyWageLoanItemEntry currentItem = (MonthlyWageLoanItemEntry)colView.GetFocusedRow();
                    currEntryRows.Remove(currentItem);
                    MyHelper.WriteLog(LogType.信息, "删除员工每月报账工资录入记录", currentItem.ToString<MonthlyWageLoanItemEntry>());
                    gridControl1.RefreshDataSource();

                    MonthlyWageLoanItemEntry.ClearMonthlyWageLoanItemEntry(currentItem.员工编号, year, month);
        ...
        private void bandedGridView1_CustomDrawCell(object sender, RowCellCustomDrawEventArgs e)
        {
            ...
            MonthlyWageLoanItemEntry row = bandedGridView1.GetRow(e.RowHandle) as MonthlyWageLoanItemEntry;
            if (row != null)
            {
        ...
        {
            MonthlyWageLoanItemEntry row = bandedGridView1.GetRow(e.PrevFocusedRowHandle) as MonthlyWageLoanItemEntry;
            if (row != null)
            {                
        ...
        {
            MonthlyWageLoanItemEntry item = bandedGridView1.GetRow(bandedGridView1.FocusedRowHandle) as MonthlyWageLoanItemEntry;
        ...
        private void Edit(MonthlyWageLoanItemEntry item)
        {
            EditMonthlyWageLoanItemForm form = new EditMonthlyWageLoanItemForm(item);
            //如果录入成功
