...
{
    public partial class PersonPayRateInput : IDoubleManInput
    {
        static readonly ILog log = LogManager.GetLogger(typeof(PersonPayRateInput));

        #region PersonPayRateInput

        /// <param name="id"></param>
        /// <returns></returns>
        public static PersonPayRateInput Get(Guid id)
        {
            PersonPayRateInput obj = (PersonPayRateInput)MyHelper.XpoSession.GetObjectByKey(typeof(PersonPayRateInput), id);
            return obj;
        }

        public static PersonPayRateInput Get(string number, bool isVerify)
        {
            ...
            XPCollection objset = new XPCollection(MyHelper.XpoSession, typeof(PersonPayRateInput), criteria, new SortProperty("录入时间", SortingDirection.Descending));

            if (objset.Count > 0)
                return (PersonPayRateInput)objset[0];
            else
                return null;
        }
        /// <summary>
        ...
        /// <returns></returns>
        public static PersonPayRateInput GetEditing(string empNo, bool isVerify)
        {
            GroupOperator criteria = new GroupOperator(GroupOperatorType.And,
        ...
        /// <param name="isVerify"></param>
        /// <returns></returns>
        public static List<PersonPayRateInput> GetEditingRows(bool isVerify)
        {
            List<PersonPayRateInput> rows = new List<PersonPayRateInput>();
            ...
            foreach (PersonPayRateInput input in objset)
            {
                rows.Add(input);
            }
            return rows;
        ...
        #region GetPersonPayRateInputGroup

        public static PersonPayRateInputGroup GetPersonPayRateInputGroup(string number)
        {
            PersonPayRateInputGroup inputGroup = new PersonPayRateInputGroup();
            ...
            foreach (PersonPayRateInput di in objset)
            {
                if (di.是验证录入)
                    inputGroup.B = di;
                else
        ...
        #region AddPersonPayRateInput

        public static PersonPayRateInput AddPersonPayRateInput(string number, string empNo, bool isVerify)
        {
            PersonPayRateInput item = Get(number, isVerify);
            if (item == null)
            {
                ...
                PersonPayRateInput otherItem = Get(number, !isVerify);
                if(otherItem  != null)
                {
                    item.姓名 = otherItem.姓名;
                    item.职务 = otherItem.职务;
                }
        ...
        protected override void OnSaving()
        {
            ...
            {
                if (this.生效日期 == DateTime.MinValue) throw new Exception("执行时间不能为空。");
            
                if (this.标识 == Guid.Empty) this.标识 = Guid.NewGuid();
                
                PersonPayRateInput other = GetEditing(this.员工编号, !this.是验证录入);
                this.编号 = other == null ? GetNewNumber() : other.编号;
                //如果是新编号，更新编号信息的当前序号
                this.NumberInfo.UpdateCurrentSN();

                PersonPayRateInput found = Get(this.编号, this.是验证录入);
                ...
                if (found != null && found.标识 != this.标识) throw new Exception(String.Format("该员工【{0}】已经有一条职级工资记录正在录入，不能重复。", this.员工编号));
            }
        ...
        {
            string sql = String.Format("UPDATE 个人职级工资_录入 SET 双人录入结果 = '{0}' WHERE 标识 = '{1}'", result, this.标识);
            using (SqlConnection connection = new SqlConnection(MyHelper.GetConnectionString()))
            {
                ...
                if (result == "两人录入完全一致")
                {
                    sql = String.Format("UPDATE 个人职级工资_录入 SET 生效时间 = '{0}' WHERE 标识 = '{1}'  AND 生效时间 IS NULL", DateTime.Now, this.标识);
                    YiKang.Data.SqlHelper.ExecuteNonQuery(connection, System.Data.CommandType.Text, sql);
                }
            }
        ...
        {
            //创建验证录入的记录
            AddPersonPayRateInput(this.编号, this.员工编号, !this.是验证录入);

            PersonPayRateInputGroup inputGroup = GetPersonPayRateInputGroup(this.编号);
        ...
        private string Compare(PersonPayRateInputGroup inputGroup)
        {
        ...
        public NumberInfo NumberInfo
        {
            get
            {
                if (_numberInfo == null)
                {
                    _numberInfo = NumberInfo.GetNumberInfo("个人职级工资录入");
                }
                return _numberInfo;
            }
            set
        ...
        PersonPayRateInput anotherInput = null;
        [Browsable(false)]
